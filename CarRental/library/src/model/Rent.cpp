...
Rent::Rent(const unsigned int &ID, Vehicle *vehicle, Client *client, pt::ptime &beginTime) : ID(ID), vehicle(vehicle), client(client), beginTime(beginTime)
{
    client->addRent(this);
...
Vehicle *Rent::get_vehicle() const
{
    return vehicle;
}

Client *Rent::get_client() const
{
    return client;
}
